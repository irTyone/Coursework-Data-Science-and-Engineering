...
# 基于 OLDA 推理结果的 K-Means 聚类分析

本模块在 OLDA/LDA 文档主题推理结果（`doc_topics.json`）基础上，进一步对文档进行 **聚类分析**，用于发现更高层次的 **热点话题结构**。

---

## 1. 聚类目的

- OLDA 推理得到的是：**单篇文档的主题分布**
- 实际热点往往由 **多个主题共同构成**
- 通过 K-Means，对「主题概率分布相似」的文档进行分组  
  → 得到 **话题级别的文档簇（Cluster）**
...
- `doc_topics.json`：每篇文档的主题概率分布  
- `topics_labeled.json`：主题名称与关键词（用于解释聚类）
...
- 将每篇文档表示为 **主题概率向量**
- 使用 K-Means 进行 **迭代式聚类**
- 每一轮迭代都会重新计算聚类中心并输出结果

> `k` 为初始聚类个数，最终关注的是**语义上稳定、有解释力的聚类**
...
对每个聚类，统计其内部文档的平均主题权重，选取权重最高的主题作为：
